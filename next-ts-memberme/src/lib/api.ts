...
// const historyPopCache = (config:any) => ({
//   forceUpdate: history.action === "PUSH",
//   ...config,
//   cache: true,
// });

export const getUserApi = (ran: any) => axios.get(endpoint.getTest + `/${ran}`);
...
// SECTION: auth page
export const Auth = {
  login: ({ email, password, config }: any = {}) => {
    return http.post(endpoint.postLogin, { user: { email, password } }, config);
  },
  register: ({ username, email, password, config }: any = {}) => {
    return http.post(
